...
namespace RichText {
    ...
    bool AbstractElement::is_in_boundaries(const Rect& b) {
        const auto& dims = m_ext_dimensions.getGlobalRect();
        return isInsideRectY(dims.y, b) || isInsideRectY(dims.y + dims.y + dims.h, b)
            || b.y > dims.y && b.y + b.h < dims.y + dims.y;
    }
    emfloat AbstractElement::hk_set_position(emfloat& cursor_y_pos, emfloat& x_offset) {
        m_ext_dimensions.setGlobalPosition(x_offset, cursor_y_pos);
        ...
        m_int_dimensions.setGlobalPosition(x_offset, cursor_y_pos);

        emfloat current_y_pos = cursor_y_pos;

        x_offset += m_style.h_paddings.x;
        cursor_y_pos += m_style.v_paddings.x;
        return current_y_pos;
    }
    void AbstractElement::hk_set_dimensions(emfloat last_y_pos, emfloat& cursor_y_pos, emfloat x_offset) {
        cursor_y_pos += m_style.v_paddings.y;
        emfloat h = cursor_y_pos - last_y_pos;
        emfloat w = m_window_width - x_offset - m_style.h_paddings.y - m_style.h_margins.y;
        m_int_dimensions.setGlobalDimensions(w, h);

        cursor_y_pos += m_style.v_margins.y;
        h = cursor_y_pos - last_y_pos;
        w = m_window_width - x_offset;
        m_ext_dimensions.setGlobalPosition(w, h);

        m_is_dimension_set = true;
    }
    bool AbstractElement::hk_draw_main(Draw::DrawList& draw_list, emfloat& cursor_y_pos, emfloat x_offset, const Rect& boundaries) {
        //ZoneScoped;
        bool ret = true;
    ...
    bool AbstractElement::draw(Draw::DrawList& draw_list, emfloat& cursor_y_pos, emfloat x_offset, const Rect& boundaries) {
        //ZoneScoped;
        bool ret = true;
        emfloat initial_y_pos = cursor_y_pos;
    ...
    void AbstractElement::setWindowWidth(emfloat width) {
        //ZoneScoped;
