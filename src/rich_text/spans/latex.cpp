...
namespace RichText {
    ...
    bool LatexWidget::add_chars(std::vector<WrapCharPtr>& wrap_chars) {
        ...
        //     ImVec2(5.f, 0.f),
        //     false);
        auto a = std::make_unique<Latex::LatexImage>(source, m_style.font_size * m_style.scale * Tempo::GetScaling(),
            7.f,
            m_style.font_color,
            ImVec2(1.f, 1.f),
            ImVec2(5.f, 0.f),
    ...
    void LatexWidget::hk_debug_attributes() {
        ...
        if (!m_error.empty()) {
            ImGui::TextColored(ImGui::ColorConvertU32ToFloat4(Colors::red), ("Error: " + m_error).c_str());
        }
    }
}