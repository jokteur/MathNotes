...
namespace RichText {
    ...
    bool AbstractLeafBlock::hk_build_widget(float x_offset) {
        ...
        if (m_widget_dirty) {
            if (m_widget_dirty & DIRTY_CHARS) {
                ...
                m_wrapper.setString(m_chars, false);
            }
    ...
    bool HiddenSpace::add_chars(WrapString* wrap_chars) {
        //ZoneScoped;
        bool success = true;
        m_chars->clear();
        m_is_selected = true;
        ...
        if (m_is_selected) {
            ...
            auto res = Utf8StrToImCharStr(m_ui_state, wrap_chars, m_safe_string, bounds.pre, bounds.end, m_special_chars_style, true);
            if (!res) {
                success = false;
            }
        }
